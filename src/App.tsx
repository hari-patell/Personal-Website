...
function FloatingIcon({
  ...
  return (
    <div
      className={`absolute animate-float ${delay} backdrop-blur-sm bg-orange-500/10 ${sizeClasses[size]} rounded-2xl border border-black/5 z-0`}
    >
      {children}
    </div>
  )
...
function App() {
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-black via-black to-zinc-900 text-white overflow-hidden">
      ...
      <div className="fixed inset-0 overflow-visible pointer-events-none max-w-[2560px] mx-auto">
        ...
        <div className="hidden xl:block">
          <FloatingIcon delay="animate-delay-9000" size="large">
            <Binary className="w-11 h-11 text-orange-500/40" />
          </FloatingIcon>
          <FloatingIcon delay="animate-delay-10000" size="default">
            <Code2 className="w-9 h-9 text-orange-500/35" />
          </FloatingIcon>
          <FloatingIcon delay="animate-delay-11000" size="small">
            <Terminal className="w-7 h-7 text-orange-500/45" />
          </FloatingIcon>
        </div>
      </div>

      {/* Main Content - centered with max width for wider screens */}
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-8 max-w-[2560px] mx-auto">
        <div className="w-full max-w-2xl p-8 rounded-3xl backdrop-blur-xl bg-black/5 border border-black/10 shadow-2xl relative overflow-hidden z-20">
          {/* Decorative Elements */}
